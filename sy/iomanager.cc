...
void IOManager::FdContext::triggerEvent(IOManager::Event event) {
    //SY_LOG_INFO(g_logger) << "fd=" << fd
    //    << " triggerEvent event=" << event
    //    << " events=" << events;
    SY_ASSERT(events & event);
    //if(SY_UNLIKELY(!(event & event))) {
    //    return;
    //}
...
void IOManager::idle() {
    ...
    while(true) {
        ...
        if(SY_UNLIKELY(stopping(next_timeout))) {
            SY_LOG_INFO(g_logger) << "name=" << getName()
                                     << " idle stopping exit";
            break;
        }
        ...
        listExpiredCb(cbs);
        if(!cbs.empty()) {
            //SY_LOG_DEBUG(g_logger) << "on timer cbs.size=" << cbs.size();
            schedule(cbs.begin(), cbs.end());
            cbs.clear();
        }

        //if(SY_UNLIKELY(rt == MAX_EVNETS)) {
        //    SY_LOG_INFO(g_logger) << "epoll wait events=" << rt;
        //}
        ...
        for(int i = 0; i < rt; ++i) {
            ...
            //SY_LOG_INFO(g_logger) << " fd=" << fd_ctx->fd << " events=" << fd_ctx->events
