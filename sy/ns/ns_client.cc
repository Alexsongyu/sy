...
NSClient::NSClient() {
    m_domains.reset(new sy::ns::NSDomainSet);
}

NSClient::~NSClient() {
    SY_LOG_DEBUG(g_logger) << "NSClient::~NSClient";
}
...
RockResult::ptr NSClient::query() {
    sy::RockRequest::ptr req = std::make_shared<sy::RockRequest>(); 
    req->setSn(sy::Atomic::addFetch(m_sn, 1));
    req->setCmd((int)NSCommand::QUERY);
    auto data = std::make_shared<sy::ns::QueryRequest>();
    ...
    do {
        if(!rt->response) {
            SY_LOG_ERROR(g_logger) << "query error result=" << rt->result;
            break;
        }
        auto rsp = rt->response->getAsPB<sy::ns::QueryResponse>();
        if(!rsp) {
            SY_LOG_ERROR(g_logger) << "invalid data not QueryResponse";
            break;
        }
        ...
        for(auto& i : rsp->infos()) {
            ...
            for(auto& n : i.nodes()) {
                ...
                if(!(node->getId() >> 32)) {
                    SY_LOG_ERROR(g_logger) << "invalid node: "
                        << node->toString();
                    continue;
                }
                domain->add(cmd, node);
...
bool NSClient::onConnect(sy::AsyncSocketStream::ptr stream) {
    if(m_timer) {
        m_timer->cancel();
    }
    auto self = std::dynamic_pointer_cast<NSClient>(shared_from_this());
...
void NSClient::onTimer() {
    ...
    auto rt = request(req, 1000);
    if(!rt->response) {
        SY_LOG_ERROR(g_logger) << "tick error result=" << rt->result;
    }
    sleep(1000);
    query();
}

void NSClient::onDisconnect(sy::AsyncSocketStream::ptr stream) {
}

bool NSClient::onNotify(sy::RockNotify::ptr nty,sy::RockStream::ptr stream) {
    do {
        if(nty->getNotify() == (uint32_t)NSNotify::NODE_CHANGE) {
            auto nm = nty->getAsPB<sy::ns::NotifyMessage>();
            if(!nm) {
                SY_LOG_ERROR(g_logger) << "invalid node_change data";
                break;
            }
            ...
            for(auto& i : nm->updates()) {
                ...
                for(auto& n : i.nodes()) {
                    ...
                    if(node->getId() >> 32) {
                        ...
                        SY_LOG_ERROR(g_logger) << "invalid node: " << node->toString();
                    }
                }
            }
        }
